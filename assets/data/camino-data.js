...
window.CAMINO_DATA = (() => {
  ...
  // Facteur de sinuosité : ramène le tracé géodésique à la distance réellement
  // marchée. Calculé, non deviné — il découle des deux valeurs ci-dessus.
  const SINUOSITY = TOTAL_KM / geodesicKm;

  // Distance de marche cumulée à chaque ville-étape.
  let running = 0;
  route[0].km = 0;
  for (let i = 1; i < route.length; i += 1) {
    running += segments[i - 1];
    route[i].km = running * SINUOSITY;
  }
  ...
  function locate(km) {
    ...
    const target = clamped / SINUOSITY;   // retour en distance géodésique
    ...
    for (let i = 0; i < segments.length; i += 1) {
      ...
      if (next >= target) {
        const r = segments[i] === 0 ? 0 : (target - travelled) / segments[i];
        const a = route[i].coords;
        const b = route[i + 1].coords;
        return {
          coords: [a[0] + (b[0] - a[0]) * r, a[1] + (b[1] - a[1]) * r],
          from: route[i],
          to: route[i + 1],
          ratioInLeg: r
        };
  ...
  return {
    ...
    geodesicKm, sinuosity: SINUOSITY,
